  ...
  return (
    <div className="space-y-8">
      <GlassCard className="p-4 sm:p-6">
        <h2 className="text-base sm:text-lg font-black tracking-tight mb-4">Add product</h2>
        <form onSubmit={createProduct} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              <label className="text-xs font-bold tracking-widest uppercase text-soul-muted block mb-1">Price (MMK) *</label>
              <input
                required
                type="number"
                min={0.01}
                ...
                className={inputClass}
              />
              <SalePriceHint price={form.price} onSale={form.tagSale} saleDiscountPercent={saleDiscountPercent} />
            </div>
            <div>
          ...
          <div>
            <label className="text-xs font-bold tracking-widest uppercase text-soul-muted block mb-1">Extra tags</label>
            <input
      ...
      <div>
        <h2 className="text-base sm:text-lg font-black tracking-tight mb-4">Catalog ({products.length})</h2>
        <div className="space-y-3">
          {products.map((p) => {
            ...
            const listPreview =
              listOriginal != null && onSale ? salePreview(String(listOriginal), true, saleDiscountPercent) : null;
            const qty = v ? totalStock(v.stock || {}) : 0;
            const isEditing = editingId === p.id && editForm;

            return (
              <GlassCard key={p.id} className="p-4 sm:p-5">
                {!isEditing ? (
                  <div className="flex flex-col sm:flex-row sm:justify-between gap-4 items-stretch sm:items-start">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-2">
                        {p.tags?.includes("new") && (
                          <span className="text-[10px] px-2 py-0.5 rounded-full bg-black text-white font-bold uppercase">New</span>
                        )}
                        {p.tags?.includes("sale") && (
                          <span className="text-[10px] px-2 py-0.5 rounded-full bg-red-600 text-white font-bold uppercase">Sale</span>
                        )}
                        {p.featured && (
                          <span className="text-[10px] px-2 py-0.5 rounded-full glass font-bold uppercase">Featured</span>
                        )}
                        {(p.tags || [])
                          .filter((t) => !["new", "sale"].includes(t))
                          .map((t) => (
                            <span key={t} className="text-[10px] px-2 py-0.5 rounded-full bg-neutral-100 text-soul-muted">
                              {t}
                            </span>
                          ))}
                      </div>
